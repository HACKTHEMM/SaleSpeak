...
# Configure TensorFlow logging and disable oneDNN custom ops unless explicitly enabled
os.environ.setdefault('TF_CPP_MIN_LOG_LEVEL', '2')  # 0=all, 1=INFO off, 2=WARNING off, 3=ERROR only
os.environ.setdefault('TF_ENABLE_ONEDNN_OPTS', '0')

# Prefer values from .env; optionally set from environment if present
if not os.getenv('GROQ_API_KEY'):
    env_val = load_env('GROQ_API_KEY')
    if env_val:
        os.environ['GROQ_API_KEY'] = env_val
if not os.getenv('SERP_API_KEY'):
    env_val = load_env('SERP_API_KEY')
    if env_val:
        os.environ['SERP_API_KEY'] = env_val
...
async def startup_event():
    if not os.getenv("GROQ_API_KEY"):
        os.environ['GROQ_API_KEY'] = load_env('GROQ_API_KEY') or ""
